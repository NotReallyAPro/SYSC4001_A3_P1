...
struct PCB{
    ...
    int             start_time;
    unsigned int    burst_time;         //The amount of CPU time needed for the processto complete execution.
    unsigned int    processing_time;    //The amount of CPU/IO time before the process switch states.
...
PCB add_process(std::vector<std::string> tokens) {
    ...
    process.burst_time = std::stoi(tokens[3]);
